...
void Shader::checkCompileErrors(unsigned int shader, const Chars &type)
{
      int success;
    char infoLog[1024];
    if (type != "PROGRAM")
...
int  Shader::getUniform(const Chars &name)
{
    return getUniformLocation(name);
}
int Shader::getAttribute(const Chars &name) 
{
    return getAttribLocation(name);
}
int Shader::getUniformLocation(const Chars &uniformName) const
{
...
bool Shader::addUniform(const Chars &name)
{
    int location = -1;
...
bool Shader::addAttribute(const Chars &name)
{
...
void Shader::SetInt(const Chars &name, int value) 
{
    int id = getUniform(name);
    if (id != -1) glUniform1i(id, value);
}

void Shader::SetMatrix4(const Chars &name, const float *value)
{
     int id = getUniform(name);
    if (id != -1)
...
void Shader::SetMatrix3(const Chars &name, const float *value)
{
    int id = getUniform(name);
    if (id != -1)
