...
int Mouse::lastMouseX = 0;
int Mouse::lastMouseY = 0;
...
void Mouse::Update()
{
    ...
    deltaPosition.x = static_cast<float>(mouseX - lastMouseX);
    deltaPosition.y = static_cast<float>(mouseY - lastMouseY);
    lastMouseX = mouseX;
    lastMouseY = mouseY;
...
void Mouse::setMousePosition(int x, int y, int xrel, int yrel)
{
   // LogInfo("setMousePosition(%i, %i, %i, %i)", x, y, xrel, yrel);
        ...
        currentPosition.x = static_cast<float>(x);
        currentPosition.y = static_cast<float>(y);
        relativePosition.x= static_cast<float>(xrel);
        relativePosition.y= static_cast<float>(yrel);
...
void Mouse::SetCursorRelative(bool state)
{
    if (state)
...
void Mouse::SetVisible(bool state)
{
    if (state)
        SDL_ShowCursor(0);
    else
...
u32 Mouse::GetMouseState(int *x, int *y)
{
    return SDL_GetMouseState(x, y);
}

u32 Mouse::GetMouseStateRelative(int *x, int *y)
{
    return SDL_GetRelativeMouseState(x, y);
}

u32 Mouse::GetMouseStateGlobal(int *x, int *y)
{
